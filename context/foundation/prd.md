---
project: "# TODO: project name — see Open Questions"
version: 1
status: draft
created: 2026-05-27
...
<!-- prd.md — generated by /10x-prd from context/foundation/shape-notes.md (2026-05-27) -->
<!-- DO NOT edit manually; re-run /10x-prd to regenerate, or edit shape-notes.md and re-run -->
...
Istniejące narzędzia (Pocket, Raindrop, Obsidian) zakładają że użytkownik najpierw projektuje system kategorii i tagów, potem zaczyna zapisywać. Ten produkt odwraca ten porządek: capture jest natychmiastowy i zero-friction, a struktura wyłania się automatycznie w tle — system uczy się użytkownika, proponuje i tworzy kategorie, i pozwala im ewoluować bez ręcznego sprzątania. Konfiguracja własnych narzędzi produktywności jest barierą dla osób niezaawansowanych technicznie; ten produkt ma być dostępny od pierwszego kliknięcia.
...
Użytkownik może zapisać link w ≤ 2 kliknięciach (rozszerzenie przeglądarki lub wiadomość do bota w komunikatorze), zobaczyć automatycznie wygenerowany micro-opis oparty na zawartości strony, i odnaleźć ten link tygodnie później przez zapytanie w naturalnym języku — bez pamiętania tytułu ani URL.
...
- FR-005: System obsługuje strony niemożliwe do pobrania bez utraty linka. Priority: must-have
  > Socrates: "Sygnalizuje użytkownikowi" przy każdym błędzie = szum informacyjny + wrażenie broken product. Rezolucja: capture flow (extension, bot) jest całkowicie milczący na błąd pobierania treści — link zostaje zapisany bez opisu. Błąd jest sygnalizowany wyłącznie wizualnie w głównej aplikacji (oznaczenie linka). Priorytet capture = szybkość i cisza.

### Organizacja

- FR-006: System proponuje strukturę kategorii użytkownikowi po zebraniu wystarczającej liczby linków, bazując na predefined templates automatycznie dopasowanych do profilu zapisanych treści. Priority: must-have
  > Socrates: Tworzenie kategorii upfront = "Obsidian trap" — użytkownik musi zaprojektować system zanim skorzysta z produktu. Rezolucja: major revision — brak wymagania tworzenia kategorii upfront. Aplikacja przechowuje kilka predefined templates struktury kategorii. Po zebraniu N linków system analizuje bazę i dopasowuje najlepiej pasujący template, który proponuje użytkownikowi. Użytkownik może zaakceptować, zmodyfikować lub odrzucić propozycję. Własne kategorie z meta-instrukcjami użytkownik może dodawać w dowolnym momencie jako rozwinięcie zaproponowanej struktury.

- FR-007: Użytkownik może przeglądać swoją bazę linków per-kategoria, w widoku "wszystkie" oraz przez interfejs wyszukiwania w naturalnym języku. Priority: must-have
  > Socrates: Bez kategorii widok "wszystkie" = chronologiczna lista identyczna z Pocket. Rezolucja: micro-opisy generowane po dodaniu linka + interfejs wyszukiwania jako główny sposób nawigacji (użytkownik zadaje pytanie, system zwraca pasujące linki). Wartość widoczna od pierwszego linka — bez potrzeby istnienia kategorii.

- FR-008: Użytkownik może wykonać świadomą akcję zamknięcia linka z listy "na później" — wybierając jeden z trzech trybów: (1) "pochłonięte — zachowaj w bibliotece" (link przechodzi do stanu referencyjnego, pozostaje dostępny i przeszukiwalny), (2) "pochłonięte — zamknij" (link usuwany z bazy, event odnotowany w statystykach), (3) "odrzuć" (link usuwany bez czytania, event odnotowany w statystykach). Każda akcja zamknięcia oferuje opcjonalne pole notatki. Priority: must-have
  > Rewizja (sesja 2): oryginalny FR-008 miał 2 stany (nieprzeczytane/przeczytane). Rozszerzony do 4-stanowego lifecycle po analizie wartości dla dashboardu. Kluczowe ustalenia: (a) "pochłonięte_zamknięte" i "odrzucone" są identyczne pod kątem przechowywania — permanentne usunięcie linka — różnią się wyłącznie w liczniku statystyk; (b) tarcie przy przejściu jest CELOWE — wymusza świadome zarządzanie listą; (c) kliknięcie linka z listy "na później" NIE zmienia stanu — tylko dodaje wizualne oznaczenie odwiedzenia i rejestruje timestamp ostatniej wizyty; (d) notatka przy akcji zamknięcia jest opcjonalna.

### Dashboard & Statystyki

- FR-012: Użytkownik może zobaczyć prosty dashboard aktywności per kategoria: liczbę linków w stanie "na później", liczbę akcji zamknięcia per typ (pochłonięte_zachowane / pochłonięte_zamknięte / odrzucone) w wybranym przedziale czasowym (ostatnie 7d / 30d / all-time), oraz datę ostatniego odwiedzenia w danej kategorii. Priority: nice-to-have
  > Socrates: Dashboard z pustymi danymi przez pierwsze 2 tygodnie może sprawiać wrażenie broken product. Rezolucja: FR-012 jako nice-to-have — dane zbiera FR-008 od dnia 1 (stany i eventy są w modelu danych od startu), dashboard można dodać gdy jest wystarczająco danych żeby miał sens. Jeśli nie starczy czasu w 3-tygodniowym MVP, dashboard wchodzi w sprint post-MVP. Model danych nie wymaga zmian retroaktywnych.

### Search

- FR-009: Użytkownik może wyszukać link zapytaniem w naturalnym języku i otrzymać pasujące wyniki. Priority: must-have
  > Socrates: Słaba jakość opisów (niescrapowalne linki) = złe wyniki = użytkownik obwinia search. Rezolucja: FR stoi; ryzyko odnotowane. Mitigacja: trzypoziomowa strategia pobierania treści (FR-004) maksymalizuje pokrycie opisami. Osobny wskaźnik jakości opisów do monitorowania po launch.
...
- **Potwierdzenie zapisu.** Użytkownik widzi potwierdzenie że link trafił do bazy w ≤ 2 sekundy od wysłania. Potwierdzenie zapisu pojawia się od razu; micro-opis i kategoryzacja są dostarczane osobno i nie blokują potwierdzenia.

- **Niezawodność zapisu.** Każdy link który dotrze do systemu jest zapisywany — awaria pobierania treści nie może skutkować utratą linka. Linki bez opisu są wizualnie odróżnialne w interfejsie przeglądania. Użytkownik może ręcznie wprowadzić lub edytować opis dla dowolnego linka.

- **Dostępność funkcji podstawowych.** Zapisywanie, przeglądanie i ręczna edycja linków są dostępne niezależnie od stanu funkcji AI (pobieranie treści, kategoryzacja, wyszukiwanie). Awaria funkcji AI degraduje tę funkcję — nie wyłącza aplikacji.
...
- URL linka
- Zawartość strony (pobrany tekst, napisy dla materiałów wideo)
- Meta-instrukcje kategorii: opis co do niej trafia + oczekiwany styl i głębokość opisu
- Wyuczony styl użytkownika: dwa zbiory przykładów (opisy które się podobały / nie podobały), na podstawie których system uczy się personalnego głosu — preferowany styl nie jest artykułowany regułą, tylko demonstrowany przykładami
...
- Target: ciągłe zarządzanie strukturą kategorii przez system; explicit feedback loop "lubię / nie lubię opis"; personalizacja trudności/nastroju treści do stanu użytkownika

**Reguła zamknięcia (celowe tarcie):**
System celowo wymusza świadome przejście przy zamykaniu linka: użytkownik wybiera jeden z trzech trybów zakończenia i ma opcję dopisania notatki. Kliknięcie linka z listy "na później" nie zmienia jego stanu — jedynie rejestruje wizytę. Zamknięcie jest zawsze jawną decyzją, nigdy automatyczną. Dane z akcji zamknięcia budują historię aktywności per zainteresowanie.
...
**Kanały dostępu (MVP):** rozszerzenie przeglądarki (desktop) + bot w komunikatorze (mobile).
...
- **Brak natywnej aplikacji mobilnej.** Mobile capture obsługuje bot w komunikatorze; aplikacja webowa nie oferuje trybu offline ani dostępu z home screen.
- **Brak pełnego archiwizowania treści stron.** Zapisujemy link i micro-opis — nie pełną kopię strony. Jeśli strona zniknie, micro-opis jest jedynym śladem treści.
- **Brak własnego systemu billing i abonamentu.** MVP działa w modelu BYOK — użytkownik podpina własny klucz API serwisu AI; operator nie zarządza kosztami ani płatnościami użytkowników.
...
- **Brak trybu rozszerzonego zapisu w MVP.** Opcjonalny formularz capture z większą kontrolą użytkownika (priorytet, notatka przy zapisie, ręczna kategoria) = v2+. Domyślny 2-klikowy zapis jest jedynym trybem w MVP.

## Open Questions

1. **Nazwa projektu** — TBD przez użytkownika. Blokada: nie (wpływa wyłącznie na branding, nie na funkcjonalność MVP).

2. **Platforma bota do mobile capture (FR-002)** — Telegram wybrany ze względu na łatwość integracji, ale użytkownik personalnie z niego nie korzysta. Do weryfikacji: która platforma (Telegram / WhatsApp / inny) jest (a) najłatwiejsza dewelopersko i (b) używana przez pierwszą grupę testową z Discorda. Blokada: nie — decyzja może być zmieniona bez wpływu na architekturę.

3. **Zbieranie metadanych do QA algorytmów kategoryzacji** — przy większej grupie użytkowników przydatne do weryfikacji czy routing i opisy działają poprawnie. Do decyzji: (a) czy w ogóle, (b) jakie dane, (c) czy anonimizowane. Blokada: nie — MVP działa bez tego; decyzja po obserwacji pierwszej grupy testowej.

4. **target_scale.qps** — nie omawiane podczas shapingu. Dla MVP z ~100 użytkownikami oczekiwane obciążenie jest niskie. Do potwierdzenia przy wyborze stacku. Blokada: nie.

5. **target_scale.data_volume** — nie omawiane podczas shapingu. Dla MVP model danych to linki + micro-opisy + metadane — mały wolumen. Do potwierdzenia przy wyborze stacku. Blokada: nie.
